...
OpenClaw 有 29.9 万 GitHub stars。创始人被 OpenAI 收编，负责"下一代个人 Agent"。

我用了一段时间。结论：**没有想象中那么大用**。

核心差异只有一个：能通过 Telegram/WhatsApp 控制。除此之外，和 Claude Code 基本一样。
...
**参与焦虑。** AI 浪潮来了，不想被落下。29 万 stars 里，有多少是"先 star 了再说"？点个 star，心理上就"参与"了。不需要真的用，不需要深度理解，一个点击就够了。
...
**开源滤镜。** 开源 = 好，这个心理预设太强了。29 万 stars 很大程度上是"支持开源"的情绪投票，不是对产品能力的认可。
...
这些问题不致命，但说明项目还在快速迭代中。29 万 stars 不代表成熟度——可能是"尝鲜型用户"多，真正深度用的少。
...
OpenAI 收购 OpenClaw 团队后，可能推出更强的能力。但就目前而言，OpenClaw 更像是一个"被风口推起来的产品"——它填补了一个真实的细分需求（后台 Agent + 社交入口），但这个需求没有 29.9 万 stars 看起来那么大。
